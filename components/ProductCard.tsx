      ...
      >
        ...
        <div className="relative w-full aspect-[3/4] bg-[#EFECE6] overflow-hidden cursor-pointer rounded-t-xl" onClick={handleImageClick}>
          ...
          <div className="relative w-full h-full overflow-hidden">
            <img src={product.frontImage} alt={`${product.name} Front`} referrerPolicy="no-referrer" onLoad={() => setImageLoadedFront(true)} className={`absolute inset-0 w-full h-full object-cover object-center transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${isBlackSelected ? 'product-color-black' : ''} ${showBack ? 'opacity-0 scale-105' : imageLoadedFront ? 'opacity-100 group-hover:scale-105' : 'opacity-0'}`} />
            <img src={product.backImage} alt={`${product.name} Back`} referrerPolicy="no-referrer" onLoad={() => setImageLoadedBack(true)} className={`absolute inset-0 w-full h-full object-cover object-center transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${isBlackSelected ? 'product-color-black' : ''} ${showBack && imageLoadedBack ? 'opacity-100 group-hover:scale-105' : 'opacity-0 scale-105'}`} />
          </div>
