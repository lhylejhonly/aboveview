...
export function StoreHeader({ onOpenLogin, onOpenProfile }: { onOpenLogin?: () => void; onOpenProfile?: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#DDD9D2] bg-[#FCFBF9] text-[#242220] shadow-[0_2px_12px_rgba(31,29,27,0.06)]">
      <div className="relative mx-auto flex h-[64px] w-full max-w-[1920px] items-center justify-between px-2 sm:h-20 sm:px-6 md:px-10 lg:px-16">
        ...
        <nav className="ml-auto flex min-w-0 items-center justify-end gap-0.5 sm:h-full sm:gap-4 md:gap-6 lg:gap-8" aria-label="Store actions">
          ...
          <button onClick={scrollToCollection} className="p-2 transition-colors hover:text-[#8C806D]" title="Search the collection" aria-label="Search the collection"><Search className="h-5 w-5 stroke-[1.35] sm:h-6 sm:w-6" /></button>
        </nav>
      </div>
    </header>
  );
