...
export function CustomerLoginModal({ onClose }: CustomerLoginModalProps) {
  const [resetMode, setResetMode] = useState<'login' | 'request' | 'update'>('login');
  ...
  const requestPasswordReset = async (event: React.FormEvent) => {
    ...
    if (result.error) setError(result.error.message);
    else setMessage('Password reset link sent. Check your email and open the link to choose a new password.');
    setLoading(false);
  };
  ...
  const updatePassword = async (event: React.FormEvent) => {
    ...
    if (result.error) setError(result.error.message);
    else { setMessage('Password updated. You can now sign in with your new password.'); setPassword(''); setResetMode('login'); }
    setLoading(false);
  };

  if (resetMode !== 'login') return <PasswordResetView mode={resetMode} email={email} password={password} error={error} message={message} loading={loading} setEmail={setEmail} setPassword={setPassword} onSubmit={resetMode === 'request' ? requestPasswordReset : updatePassword} onBack={() => { setResetMode('login'); setError(''); setMessage(''); }} onClose={onClose} />;
...
function PasswordResetView({ mode, email, password, error, message, loading, setEmail, setPassword, onSubmit, onBack, onClose }: { mode: 'request' | 'update'; email: string; password: string; error: string; message: string; loading: boolean; setEmail: (value: string) => void; setPassword: (value: string) => void; onSubmit: (event: React.FormEvent) => void; onBack: () => void; onClose: () => void }) {
  const update = mode === 'update';
  return <AnimatePresence><div className="fixed inset-0 z-[80] flex items-center justify-center overflow-y-auto bg-[#171513]/75 p-3 backdrop-blur-sm sm:p-5"><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 18 }} className="w-full max-w-lg rounded-xl border border-[#D6CFC7] bg-[#F6F3EE] shadow-2xl"><div className="flex items-center justify-between border-b border-[#DDD7CF] bg-[#FCFBF9] px-5 py-4 sm:px-7"><div><p className="text-[9px] font-bold uppercase tracking-[.28em] text-[#8C806D]">Above Apprl · Customer access</p><h2 className="mt-1 font-sans text-lg font-bold uppercase tracking-[.1em] text-[#2D2926]">{update ? 'Choose a new password' : 'Reset your password'}</h2></div><button onClick={onClose} className="rounded-full p-2 text-[#6F6963] hover:bg-[#EEEAE4]" title="Close"><X className="h-5 w-5" /></button></div><form onSubmit={onSubmit} className="px-5 py-8 sm:px-10 sm:py-10"><p className="mb-6 text-sm leading-6 text-[#6F6963]">{update ? 'Your email has been confirmed. Choose a new password for your customer account.' : 'Enter your email and we’ll send you a secure password reset link.'}</p>{!update && <label className="block text-[10px] font-bold uppercase tracking-[.14em] text-[#6F6963]">Email address<input required type="email" value={email} onChange={event => setEmail(event.target.value)} className="mt-1.5 min-h-12 w-full rounded-sm border border-[#D9D3CA] bg-[#FCFBF9] px-3.5 py-3 text-sm text-[#2D2926] outline-none focus:border-[#8C806D]" /></label>}{update && <label className="block text-[10px] font-bold uppercase tracking-[.14em] text-[#6F6963]">New password<input required minLength={6} type="password" value={password} onChange={event => setPassword(event.target.value)} className="mt-1.5 min-h-12 w-full rounded-sm border border-[#D9D3CA] bg-[#FCFBF9] px-3.5 py-3 text-sm text-[#2D2926] outline-none focus:border-[#8C806D]" /></label>}{error && <p role="alert" className="mt-5 rounded-sm border border-[#E5BDB0] bg-[#FFF5F1] p-3 text-xs leading-5 text-[#9A4D3A]">{error}</p>}{message && <p role="status" className="mt-5 rounded-sm border border-[#CFDCC8] bg-[#F0F5ED] p-3 text-xs leading-5 text-[#4D633E]">{message}</p>}<button disabled={loading} className="mt-7 flex min-h-13 w-full items-center justify-center rounded-sm bg-[#2D2926] px-5 text-xs font-bold uppercase tracking-[.17em] text-[#F4F1EE] disabled:opacity-60">{loading ? 'Please wait...' : update ? 'Update password' : 'Send reset link'}</button><button type="button" onClick={onBack} className="mx-auto mt-5 block text-[10px] font-bold uppercase tracking-[.12em] text-[#8C806D] hover:text-[#2D2926]">Back to sign in</button></form></motion.div></div></AnimatePresence>;
}
